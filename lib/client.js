...
class tunnelClient extends events{
	...
	get tunnel(){this.tunnelHelper.getTunnel(this.connection);}
	get tunnelID(){this.tunnelHelper.tunnelID.get(this.connection);}
	get tunnelCreated(){this.tunnelHelper.hasTunnel(this.connection);}
	requestTunnel(ws,options){
		if(this.tunnelCreated)
			throw(Error('Tunnel already created'));
		let msg;
