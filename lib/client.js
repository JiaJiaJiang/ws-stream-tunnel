...
const events=require('events'),
		...
		debug=require('debug')('ws-stream-tunnel:client'),
		detectNetworkChanged=require('./networkChanged.js').detect;
...
class tunnelClient extends events{
	constructor(options){
		...
		}).once('_wsclose',(code,reason)=>{
			let tunnel=this.tunnelHelper.getTunnel(this.connection);
			(code!==1000)&&tunnel&&this.tunnelHelper.toBroken(tunnel,'connection closed');
		}).on('idle',()=>{
	...
	requestTunnel(ws,options){
		...
		if(this.tunnelID && options.keepBrokenTunnel){//if tunnelID exists and the tunnel was kept
			debug('use tunnel',this.tunnelID);
...
class connectionMng extends events{
	...
	connect(addr,callback){
		if(this.connecting || this.closed || this.ws)return false;
		...
		debug('connecting to:',this.addr);
		let ws=this.ws=new Websocket(this.addr,this.options.ws);
		ws.once('open',()=>{
			debug('ws','open');
			//socket settings
			let socket=ws._socket;
			socket.on('networkChanged',()=>{
				this._closeWs(1011,'network changed');
			});
			detectNetworkChanged(socket);
			...
			this.emit('_wsclose',ws,code,reason);
		}).on('message',data=>{
			this._active();
			this._living();
	...
	send(data,opt,callback){
		...
		if(!this.ws){
			callback&&setImmediate(callback,Error('not connected'));
			return;
		}
