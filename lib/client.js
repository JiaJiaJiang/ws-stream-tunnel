...
class connectionMng extends events{
	constructor(options){
		...
		this._idleTimer=setInterval(()=>{
			if(this._disableIdle || !this.connected || this.closed)return;
	...
	connect(addr,callback){
		...
		if(this.ws)this._closeWs(3005,'starting a new connection');
...
class tunnelClient extends events{
	constructor(options){
		...
		this.connectionMng.on('_wsopen',ws=>{
			...
			this.close(reason);
			this.tunnel.close('connection closed :'+reason);
		});
		...
		.on('stream_open',()=>{this.connectionMng._disableIdle=true;})
		.on('stream_close',()=>{if(this.tunnel.subStreams.size===0)this.connectionMng._disableIdle=false;});
	...
	close(code,reason){
		...
		this.connectionMng.close(1000,'client closed');
		this.emit('close');
	}
}
