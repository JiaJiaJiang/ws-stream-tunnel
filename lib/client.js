	...
	constructor(options){
		...
		if(this.options.idleTimeout>0){
			this._idleTimer=setInterval(()=>{
				if(this._disableIdle || this.connecting || this.closed || this.idle || this.ws.readyState!==1)return;
...
class tunnelClient extends events{
	constructor(options){
		...
		.on('stream_close',()=>{if(this.tunnel.streams.size===0)this.connectionManager._disableIdle=false;});
