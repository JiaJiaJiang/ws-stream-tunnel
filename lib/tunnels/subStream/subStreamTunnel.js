...
class subStreamTunnel extends tunnelManager{
	...
	_newSubStream(sid,stream){
		...
		sub._send=(data,encoding,callback)=>this._dataMsg(sub.sid,data,callback);
	...
	_dataMsg(sid,data,callback){
		//sned data frame
		this._send(0,sid,data,(err)=>callback(err||null));
	}
	receive(data){//message from websocket
		...
		switch(frame.type){
			...
			case 1:{//start frame
				...
				sub=this._newSubStream(sid,new ioStream({highWaterMark:this.highWaterMark}),this);
				this._send(4,sid);//send started frame
				sub._open();
				return;
			}
			case 2:{//close frame
			...
			case 6:{//error frame
				...
				switch(sub.state){
					case SubStream.STARTING:{
						...
						sub._retry++;
						this.subStreams.delete(sid);//remove the old sid
						let nsid=this._genSubStreamID();//re-generate an id
						sub.sid=nsid;
						this._send(1,nsid);//restart the stream
						this.subStreams.set(nsid,sub);//set the new sid
						break;
					}
