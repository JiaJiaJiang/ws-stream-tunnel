...
stream frame
	...
	3-4		nothing for now
	5-7	 	bytes count following,1-4 bytes
*/
...
class SubStream extends events{
	constructor(sid,stream,tunnel){
		...
		stream._write=(data,encoding,callback)=>{
			tunnel._send(0,this.sid,data,callback);
		};
		stream.on('error',e=>{
			if(e.name==='RemoteError'){
				...
				stream._error=e;
				tunnel._errorMsg(sid,e.message);
			}
			if(!(stream._readableState.destroyed||stream._writableState.destroyed)){
				stream.destroy(e);
			}
		}).once('finish',e=>{//send close stream frame
			//this event will be emit twice if the stream is a Duplex,I don;t know why.
			//so if this.state===SubStream.CLOSED,do nothing
