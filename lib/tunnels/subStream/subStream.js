...
class SubStream extends events{
	...
	static frameParser(buffer){//parse the frame
		let idBytes=buffer[0]&0b111,type=buffer[0]>>>5,sid=0;
		if(idBytes!==0){//calc the id
			for(let i=idBytes,bytes=idBytes;i--;bytes--){//get id
				sid+=((buffer[bytes]<<((bytes-1)*8))>>>0);
			}
		}
		return {type,sid,data:(buffer.byteLength>1+idBytes)?buffer.slice(1+idBytes):null};
	}
	static frame(type,sid,data){//type(number) sid(number). build a frame buffer
		...
		for(let byte=idBytes;byte;byte--){
			buf[byte]=(sid>>>((idBytes-byte)*8))&0xFF;
		}
