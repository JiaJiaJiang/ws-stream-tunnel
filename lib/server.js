...
class tunnelServer extends Websocket.Server{
	constructor(options,callback){//the optiosns obj is used as Websocket server's options
		...
		this._tunnelToWs=new WeakMap();//tunnel=>ws
		this._wsToReq=new WeakMap();//ws=>req
		...
		this.on('connection',(ws,req)=>{
			...
			ws.on('message',data=>{
				ws._live=Date.now();
				if(data.byteLength){
					if(ws.tunnel)ws.tunnel.receive(data);
					else this._sendTunnelError(ws,4006,'binary data received before creating tunnel');
					return;
				}
				try{
				...
				switch(c._){
					case 'create':{//tunnel create msg
						if(ws.tunnel){//tunnel exists
							this._sendTunnelError(ws,4001,'tunnel already created');
							return;
						}
						//create tunnel
						if(!this.allowedStreamMode.has(c.mode)){//not an allowed mode
							this._sendTunnelError(ws,4002,'not allowed mode:'+c.mode);
							return;
						}
						if(!(c.mode in Tunnel)){
							this._sendTunnelError(ws,4003,'not supported tunnel type:'+c.mode);
							return;
						}
					...
					case 'use':{//tunnel reuse msg
						...
						if(!tunnel){
							this._sendTunnelError(ws,4004,`tunnel of id '${c.id}' not exists`);
							break;
						}
						//reset the tunnel
						tunnel.send=ws.send.bind(ws);
					...
					default:{
						this._sendTunnelError(ws,4005,'wrong configuration msg');
						break;
					}
				}
			}).on('ping',()=>{
	...
	getWsOfTunnel(tunnel){
		return this._tunnelToWs.get(tunnel);
	}
	getReqOfWs(ws){
		return this._wsToReq.get(ws);
	}
	close(){
