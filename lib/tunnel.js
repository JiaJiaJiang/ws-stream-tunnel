...
class Tunnel extends events{
	...
	clearTunnelStream(){}//to be overwritten by sub class
	close(reason){
		if(this.closed===true)return false;
		this.closed=true;
		this.clearTunnelStream(reason);
		this.cacheStream.destroy();
...
class tunnelHelper extends events{
	constructor(){
		...
		this._shouldCloseEmitted=false;
		this._shouldClose=()=>{
			if(this._shouldCloseEmitted)return;
			this._shouldCloseEmitted=true;
			this.emit('should-close');
		};
		process.once('exit',this._shouldClose);
		process.once('SIGINT',this._shouldClose);
	}
	handle(obj,data){
		...
		try{
			...
			// debug('cannot parse data',e,data);
		...
		switch(c._){
			...
			case 'error':{
				...
				if(c.code>=4000 && c.code<=4007){
					debug('tunnel error:',c.code,c.msg);
					this.closeTunnel(obj,c.msg);
					this.emit('error',obj,c.code,c.msg);
					return;
				}
				debug('unexpected tunnel error',c.code,c.msg);//for debug
				break;
			}
			default:{
	...
	createTunnel(obj,opts){
		...
		tunnel.once('close',reason=>{
			...
			debug('tunnel close',tunnel.id);
			this.emit('tunnel_close',tunnel);
		})._open();
		debug('tunnel open',tunnel.id);
	...
	_bindTunnel(obj,tid){
		...
		let preObj;
		if(preObj=this.tunnelToObj.get(tunnel)){
			if(preObj===obj){//the same obj
				return;
			}
			this._clearTunnelMap(tunnel.id);
		}
	...
	clear(reason='unknown'){
		process.removeListener('exit',this._shouldClose);
		process.removeListener('SIGINT',this._shouldClose);
